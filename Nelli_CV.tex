...
relativistic magneto-hydro dynamics on HPC and exascale machines.}
  \resumeItem{Expert on utilizing task-based (asynchonous) parallelism to
  achieve 3x speedup when solving partial differential equations on exascale
  computing resources.}
  \resumeItem{Visualized $\sim$1TB of output from simulations using Paraview and its
  Python scripting framework.}
  \resumeItem{Implemented detailed memory diagnostics and corresponding visualization
  tools in Python. Used to reduce memory usage by 5x and find numerous bugs.}
